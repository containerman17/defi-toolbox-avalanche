...
import type { StoredPool, PoolType } from "./types.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const DATA_DIR = join(__dirname, "..", "data");

/** Map from provider name to pool type ID */
const POOL_TYPE_MAP: Record<string, number> = {
...
export function parsePools(content: string): { headBlock: number; pools: StoredPool[] } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (poolTypeId === undefined) continue;
    ...
    pools.push({
      address,
      providerName,
      poolType: poolTypeId as PoolType,
      tokens,
      latestSwapBlock,
      extraData,
    });
...
export function loadPools(path?: string, limit?: number): StoredPool[] {
