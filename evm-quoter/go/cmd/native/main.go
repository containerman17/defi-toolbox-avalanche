...
func newWSFetcher(url string) (*wsFetcher, *harness.StateDB, error) {
...
func (f *wsFetcher) readLoop(state *harness.StateDB) {
	for {
...
func applyParsedOverrides(base *harness.StateDB, overrides []parsedOverride) *harness.StateDB {
	if len(overrides) == 0 {
		return base
	}
...
func applyOverrides(base *harness.StateDB, raw map[string]stateOverrideEntry) *harness.StateDB {
