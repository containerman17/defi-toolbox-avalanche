...
func main() {
	...
	for _, p := range eligible {
		...
		// Try both directions — pool is valid if either direction returns non-zero
		valid := false
		for _, dir := range [][2]int{{0, 1}, {1, 0}} {
			calldata := pathfinder.EncodeSwapSingle(p.Address, p.PoolType, p.Tokens[dir[0]], p.Tokens[dir[1]], amountIn)
			cs.Reset()
	...
	for _, id := range []int{-1, 0, 1, 2, 3, 4, 5} {
		s := byFormula[id]
		if s == nil {
			continue
		}
	...
	lines = append(lines, "# 0=V2, 1=PharaohV1, 2=V3, 3=LFJV2, 4=Algebra, 5=DODO, -1=invalid")
	...
	if doWrite {
		...
		err := os.WriteFile(registryPath, []byte(strings.Join(lines, "\n")+"\n"), 0o644)
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to write: %v\n", err)
			os.Exit(1)
		}
		fmt.Fprintf(os.Stderr, "\nWrote %s (%d entries)\n", registryPath, len(results))
		fmt.Fprintf(os.Stderr, "Verify: go run ./cmd/benchmark/ --state-server ws://localhost:7449 --correctness\n")
	} else {
		fmt.Fprintf(os.Stderr, "\nDry run — pass --write to save (%d entries)\n", len(results))
	}
}
