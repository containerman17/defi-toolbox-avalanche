...
- When `extractPoolHops` returns a hop chain that doesn't start at the input
  token or end at the output token, the benchmark now scans the pool catalog
  for a bridging pool instead of marking the tx unsupported. Covers cases like
...
- Found 2083 overquotes. Bulk-blacklisted 2734 pools to -1.
- **This is wrong.** 2734 out of 8732 registered pools (33%) are now
  ...
  pools blacklisted. The `token_overrides.json` data hasn't changed — the
  same overrides that worked before are being applied. Something in the
  new `ApplyTokenOverrides` / `EVMCallOn` / lightclient StateView path
  is different from the old statedb-based setup, causing EVM calls to
  revert where they previously succeeded. Root cause is unknown — needs
  investigation before reverting the blacklist.
- Result: 0 overquotes, 96.7% match — but at the cost of killing a third
  of the pool coverage.
...
- `lightclient/cmd/verify/` — executes recent blocks locally, compares storage
  diffs against debug trace. 5000/5000+ blocks match. Skips newly created
  contracts (trace reports zeros for them). Timing: fetch ~2ms, exec p50=25ms
  p90=103ms, trace 8-100ms (verification only).
